...
Defines the `quality-gate-fast` workflow — a pre-commit quality gate that runs all checks except mutation testing — and updates the `quality-gate` workflow to include all fast-gate steps plus mutation, so both workflows share the same base set of tracked checks.
