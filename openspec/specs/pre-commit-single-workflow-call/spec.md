...
Defines the pre-commit hook contract after migrating from a multi-step shell pipeline to a single `swamp workflow run quality-gate-fast` call, and removes the now-superseded `check-codescene-health.js` script.
