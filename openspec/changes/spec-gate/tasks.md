...
- [ ] 2.7 Validate end-to-end: generate-features → commit feature files → tests step runs cucumber → passes
...
- [ ] 7.5 Verify the full pipeline: cucumber runs, one scenario passes, all others skipped, gate is green
- [ ] 7.6 Commit all feature files (as deliverables, not generated artifacts)

## 8. Removal of OpenSpec

- [ ] 7.1 Complete `fix-whitelist-bypass-promoted-keyword` change under openspec before migrating
- [ ] 7.2 Remove openspec skill files from `.claude/skills/openspec-*/`
- [ ] 7.3 Update `CLAUDE.md` to reference spec-gate skills instead of openspec
- [ ] 7.4 Document that `openspec/specs/` markdown files are now rendered outputs (generated by model), not source of truth
