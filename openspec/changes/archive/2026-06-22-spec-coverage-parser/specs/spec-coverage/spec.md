...
The parser SHALL search all `tests/**/*.test.js` files for `it('Scenario: <name>')` occurrences. A scenario is considered covered when its exact name appears as the first argument to an `it()` call, regardless of quoting style (single or double quotes).
...
The parser SHALL be runnable via `npm run spec:coverage` defined in `package.json`.
